...
def get_session_history(session_id: str):
    # 这行代码会在你运行的目录下自动创建一个 memory.db 文件
    # 它会根据 session_id 自动隔离不同用户的聊天记录
    return SQLChatMessageHistory(
        session_id=session_id, 
        connection_string="sqlite:///memory.db"
    )

# 2. 设定一个 Session ID（比如代表你现在的这场对话）
# 在实际应用中，这通常是用户的 ID 或者对话窗口的 ID
...
while True:
    ...
    #
    # HumanMessage / AIMessage は LangChain が提供するメッセージ型で、
    # 「誰が発言したか」を区別するために使い分ける
